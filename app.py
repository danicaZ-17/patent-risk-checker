...
st.write("输入商品描述，系统将通过多维度评估判断专利侵权风险")
...
def evaluate_dimensions(query, patent, score):
    ...
    tech_match = sum(1 for kw in tech_keywords if kw in text or kw in query) / len(tech_keywords)
    
    # 维度2：功能特征一致性（基于功能词匹配）
    func_keywords = ["控制", "检测", "监测", "连接", "传输", "处理", "调节", "保护", "充电", "散热"]
    func_match = sum(1 for kw in func_keywords if kw in text or kw in query) / len(func_keywords)
    
    # 维度3：领域适用性（基于品类匹配）
    categories = ["家居用品", "数码配件", "车载配件", "手机配件", "个护健康", "新能源", "智能穿戴"]
    cat_match = 1.0 if patent["品类"] in query else 0.5
    
    # 综合评分以检索相似度为核心，各维度作为微调
    overall = min(score * 0.6 + tech_match * 0.2 + func_match * 0.1 + cat_match * 0.1, 0.95)
    
    return {
        "整体技术匹配度": round(min(tech_match * 0.8 + 0.2, 0.95), 3),
        "功能特征一致性": round(min(func_match * 0.8 + 0.2, 0.95), 3),
        "领域适用性": round(min(cat_match * 0.7 + 0.2, 0.95), 3)
    }
...
if st.button("🔍 开始筛查"):
    if not query or len(query.strip()) == 0:
        ...
        with status_container:
            ...
            # 步骤1：解析商品描述
            status_placeholder.info("🔍 步骤 1/3：正在解析商品描述，提取关键技术特征...")
            time.sleep(0.3)  
            
            # 步骤2：检索专利数据库
            status_placeholder.info("📚 步骤 2/3：正在检索专利数据库，匹配相关专利...")
            time.sleep(0.3)
            ...
            # 步骤3：分析风险
            status_placeholder.info("📊 步骤 3/3：正在执行多维度风险分析，生成评估报告...")
            time.sleep(0.3)
